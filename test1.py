...
def convert_time(t):
	return (int(t[:2])-8)*60+int(t[3:])
	
def tabu_search_vrpcd(G, px='Lng', py='Lat',
                      node_type='tp', quantity='quantity', pair='pair',
                      cons='consolidation', tolerance=20, L=12, k=2, a=10,
                      diversification_iter=0, capacity_cap=150,max_tabu_len=20,	capacity_p_c = 3,used_vehicles_c = 200):
	...
	while diver_iter_total < tolerance:
		if new_best ==1 or diver_iter%10==0:
			...
			temp_solution, cost= inter_route_change(routing_dic,G,dist,capacity_p_c,tabu_list,max_tabu_len,node_type,pair,capacity_cap)
			new_best=0
		else:
			temp_solution, cost=intra_route_change(routing_dic,G,dist,capacity_p_c,node_type,pair,capacity_cap)
		
		if cost < best_cost and check_temp_solution_feasible(routing_dic,G,dist,capacity_cap,quantity,node_type,pair):
			diver_iter = 0
			best_cost = cost
	...
	return best_sol.copy(), best_cost
    

	
def inter_route_change(routing_dic,G,dist,capacity_p_c,tabu_list,max_tabu_len,node_type,pair,capacity_cap):
	for u in G:
		if u in tabu_list:
			continue
		if G.node[u][node_type] == 'pickup':
			...
			current_cost_u=calculate_route_f(G,current_route,dist,capacity_p_c,capacity_cap)
			pair_u = G.node[u][pair]
			
			current_route.remove(u)
			current_route.remove(pair_u)
			best_change_move = []
			best_change_cost = 0
			...
			for target_vehicle_id in routing_dic:
				if vehicle_id_u != target_vehicle_id:
					...
					current_cost_v=calculate_route_f(G,target_route_v,dist,capacity_p_c,capacity_cap)
				
					for posi_num in range(1,len(target_route_v)):
						...
						for posi_num_pair in range(posi_num+1,len(target_route)):
							...
							current_cost_u_new=calculate_route_f(G,current_route,dist,capacity_p_c,capacity_cap)
							current_cost_v_new=calculate_route_f(G,target_route,dist,capacity_p_c,capacity_cap)
	...
	cost_route_dic = sum([calculate_route_f(G,routing_dic[vehicle_id],dist,capacity_p_c,capacity_cap) for vehicle_id in routing_dic])
	return 	routing_dic.copy(),cost_route_dic
				
def intra_route_change(routing_dic,G,dist,capacity_p_c,node_type,pair,capacity_cap):
	for u in G:
		vehicle_id_u = find_vehicle_by_node(routing_dic, u)
		current_route=routing_dic[vehicle_id_u][:]
		current_cost_u=calculate_route_f(G,current_route,dist,capacity_p_c,capacity_cap)
		...
		if G.node[u][node_type] == 'pickup':
			for posi in range(1,pair_posi+1):
				...
				current_cost_u_new=calculate_route_f(G,current_route_new,dist,capacity_p_c,capacity_cap)
...
def clarke_wright(G, routing_dic, dist, node_type, quantity, pair, capacity_cap, simplify='Y'):
		...
		for v in G:
			if G.node[u][node_type] == 'pickup' and G.node[v][node_type] == 'pickup':
				...
				if simplify=='Y':
					...
					for route_t in itertools.permutations(current_route_new):
						route = list(route_t)
						...
						if not(check_route_order(G, route,node_type,pair) and check_route_capacity(route_capacity,capacity_cap)):
							continue
	...
	for (u, v), savings in sorted_savings:
        ...
        if  vehicle_id_u != vehicle_id_v and G.node[u][node_type] == 'pickup' and G.node[v][node_type] == 'pickup':
			...
			best_route, best_route_cost = [], float('inf')
			for route_t in itertools.permutations(current_route):
				route = list(route_t)
				route.insert(0,start_node)
				#check if it is feasible
				route_capacity = calculate_route_capacity(G, route, quantity,node_type)
...
def calculate_route_f(G,route,dist,capacity_p_c,capacity_cap):
	P_time='P_time'
	D_time='D_time'
	node_type='tp'
	quantity='quantity'
	...
	if len(route)>1:
		for i in range(1,len(route)):
			...
			#route_timeline[u] = [arrive_time, leave_time]
	route_capacity=calculate_route_capacity(G, route, quantity,node_type)
	route_duration = penalty_time + leave_time + sum([max(route_capacity[u]-capacity_cap,0) for u in route_capacity]) * capacity_p_c
	if leave_time >=720:
		route_duration = float('inf')
	return route_duration
	
	
def check_route_order(G, route,node_type,pair):
	flag=[]
	for uu in route:
...
def check_temp_solution_feasible(routing_dic,G,dist,capacity_cap,quantity,node_type,pair):
	for vehicle_id in routing_dic:
		route=routing_dic[vehicle_id][:]
		route_capacity = calculate_route_capacity(G, route, quantity,node_type)
		if not(check_route_order(G, route,node_type,pair) and check_route_capacity(route_capacity,capacity_cap)):
			return False
	return True
...
for i in range(len(E)):
    G.add_node(E['Order_id'][i]+'_Pick',tp='pickup',Lng=E['Lng_Pick'][i],Lat=E['Lat_Pick'][i],quantity=E['Num'][i],
               P_time=convert_time(E['Pickup_time'][i]),D_time=convert_time(E['Delivery_time'][i]),pair=E['Order_id'][i]+'_Drop')
    G.add_node(E['Order_id'][i]+'_Drop',tp='dropoff',Lng=E['Lng_Drop'][i],Lat=E['Lat_Drop'][i],quantity=E['Num'][i],
               P_time=convert_time(E['Pickup_time'][i]),D_time=convert_time(E['Delivery_time'][i]),pair=E['Order_id'][i]+'_Pick')
 
#best_sol,best_cost = tabu_search_vrpcd(G)
